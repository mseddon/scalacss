...
object DslBase {
  ...
  final class DslCond(c: Cond, b: DslBase) {
     def apply(t: ToStyle*)(implicit u: Compose): StyleS = c applyToStyle b.mixin(t: _*)
  }
...
abstract class DslBase
  extends AttrAliasesAndValueTRules
   ...
   override protected final def mkColor(s: String) = Color(s)

   implicit final def autoDslInt  (a: Int)          : DslInt         = new DslInt(a)
   implicit final def autoDslNumI (a: Int)          : DslNum[Int]    = new DslNum[Int](a)
   implicit final def autoDslNumD (a: Double)       : DslNum[Double] = new DslNum[Double](a)
  //nline implicit final def autoDslStr  (a: String)       : DslStr         = new DslStr(a)
   implicit final def autoDslAttr (a: Attr)         : DslAttr        = new DslAttr(a)
   implicit final def autoDslAttrT(a: TypedAttrBase): DslAttrT       = new DslAttrT(a)
   implicit final def autoDslAV   (a: AV)           : DslAV          = new DslAV(a)
   implicit final def autoDslAVs  (a: AVs)          : DslAVs         = new DslAVs(a)

   implicit final def DslCond[C <% Cond](x: C): DslCond = new DslCond(x, this)

   implicit final def ToAVToAV(x: ToAV): AV = x.av
   ...
   implicit final def ToStyleStyleS         (x: StyleS)    : ToStyle = new ToStyle(x)
